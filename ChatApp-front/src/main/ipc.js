...
//确保本地存在会话
const onEnsureChatSession = () => {
    ipcMain.handle("ensureChatSession", async (e, sessionInfo) => {
        if (!sessionInfo || !sessionInfo.contactId) {
            return null;
        }
        const dbSession = await selectUserSessionByContactId(sessionInfo.contactId);
        if (dbSession) {
            await updateStatus(sessionInfo.contactId);
            dbSession.status = 1;
            return dbSession;
        }
...
const openWindow = ({ windowId, title = "ChatApp", path, width = 960, height = 720, data }) => {
