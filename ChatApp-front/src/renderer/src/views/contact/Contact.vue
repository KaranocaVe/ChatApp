...
const joinedGroupData = ref([])

const refreshMyGroupSection = () => {
  const mergedMap = new Map()
  myGroupData.value.forEach((group) => {
    mergedMap.set(group.groupId, {
      groupId: group.groupId,
      groupName: group.groupName,
      isOwner: true
    })
  })
  joinedGroupData.value.forEach((contact) => {
    const groupId = contact.contactId
  ...
  partList.value[1].contactData = Array.from(mergedMap.values())
}
...
const loadContact = async (contactType) => {
  ...
  if (contactType === 'GROUP') {
    ...
    joinedGroupData.value = result.data || []
    refreshMyGroupSection()
