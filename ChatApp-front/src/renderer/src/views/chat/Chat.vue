...
const creatingSessionIds = new Set()
const buildChatSessionInfo = async (contactId) => {
  const sessionInfo = {
    contactId,
    status: 1,
    contactType: 0,
  ...
  const isGroupContact = contactId && contactId.startsWith('G')
  if (isGroupContact) {
    ...
    sessionInfo.contactType = 1
    sessionInfo.contactName = result.data.groupName
    sessionInfo.memberCount = result.data.memberCount
    sessionInfo.sessionId = md5(contactId)
  } else {
    let result = await proxy.Request({
    ...
    sessionInfo.contactName = result.data.nickName
    const ids = [userInfoStore.getInfo().userId, contactId].sort()
    sessionInfo.sessionId = md5(ids.join(''))
  }
  return sessionInfo
}

const ensureLocalChatSession = async (contactId) => {
  if (!contactId || creatingSessionIds.has(contactId)) {
    return null
  }
  creatingSessionIds.add(contactId)
  try {
    let sessionInfo = await window.ipcRenderer.invoke('ensureChatSession', { contactId })
    if (sessionInfo) {
      return sessionInfo
    }
    sessionInfo = await buildChatSessionInfo(contactId)
    if (!sessionInfo) {
      return null
    }
    return await window.ipcRenderer.invoke('ensureChatSession', sessionInfo)
  } catch (error) {
    console.error('ensureChatSession failed', error)
    return null
  } finally {
    creatingSessionIds.delete(contactId)
  }
}
...
const sendMessage = async (contactId) => {
  ...
  if (!curSession) {
    const sessionInfo = await ensureLocalChatSession(contactId)
    if (!sessionInfo) {
      return
    }
